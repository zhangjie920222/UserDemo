...
<html>
  ...
  <body>
   ...
   <form id="frm" action="modifyUser.do" method="post">
   ...
   <table>
    <tr>
     ...
     <td>
      ...
      <input type="checkbox" name="interest" class="interest" value="篮球" />篮球
      <input type="checkbox" name="interest" class="interest" value="游戏" />游戏
      <input type="checkbox" name="interest" class="interest" value="游泳" />游泳
      <input type="checkbox" name="interest" class="interest" value="唱歌"/>唱歌
     </td>
    </tr>
     <tr>
     <td>入职时间</td>
     <td><div id="timeplug"><input type="text" name="intime" value="${user.intime}" class="laydate-icon"/></div></td>
    </tr>
     <tr>
     <td>部门</td>
     <td>
        <select id="seldepart" name="departNo">
           <option value="1" selected="${user.departno==1?'selected':false}">研发部</option>
           <option value="2" selected="${user.departno==2?'selected':false}">测试部</option>
           <option value="3" selected="${user.departno==3?'selected':false}">集成部</option>
           <option value="4" selected="${user.departno==4?'selected':false}">网络部</option>
           <option value="5" selected="${user.departno==5?'selected':false}">人事部</option>
        </select>
     </td>
    </tr>   
   </table>
    ...
    //全选
     $(function() {
           $("#checkAll").click(function() {
                $('input[name="interest"]').attr("checked",this.checked); 
            });
            var $subBox = $("input[name='interest']");
            $subBox.click(function(){
                $("#checkAll").attr("checked",$subBox.length == $("input[name='interest']:checked").length ? true : false);
           });
     });
   ...
   laydate({
    ...
    $(function(){
       ...
       var checkboxs=$("input[name=interest]");   
       //将全选按钮选中
