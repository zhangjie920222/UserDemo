...
<html>
  ...
  <body>
   <form id="frm" action="listUser.do" method="post" >
  <div id="resetformdiv">
   ...
   <div style="width:560px;margin:0 auto;margin-top:20px;" id="timet">
     ...
     &nbsp;&nbsp;<label>入职时间</label>&nbsp;&nbsp;
     <input type="text" name="time1" class="laydate-icon" value="${pageinfo.fromtime}"/>
     <label>一</label>
     <input type="text" class="laydate-icon" name="time2"  value="${pageinfo.totime}"/>
   </div>
     ...
     <tr style="background:#ccc;">
	  ...
	  <td  colspan="6" style="text-align:center;">
		...
		<input type="button" onclick="gotoPage(${pageinfo.totalPageNum})" value="尾页"/>
	         当前第<input name="pageNum" value="${pageinfo.pageNum}" id="repagenum" style="width:30px;" type="text" onkeydown="function keydown(e){e = e||event;  if(e.keyCode == 13) {gotoPage($(this).val());}return;}"/>页/共<span>${pageinfo.totalPageNum}</span>页
  ...
  <script type="text/javascript">
    ...
    function toAdduser(){
     ...
     //for chrome
      ...
      if(reshf.closed){
       ...
       //测试
      var rv = window.showModalDialog("child.do",window,"dialogWidth:350px;dialogHeight:350px");
      //for chrome 
      if (rv == undefined) {
    ...
    function toModifyUser(id){
     var openUrl = "toModifyUser.do?userid="+id+" ";//弹出窗口的url
     var iWidth=600; //弹出窗口的宽度;
     var iHeight=400; //弹出窗口的高度;
     var iTop = (window.screen.availHeight-30-iHeight)/2; //获得窗口的垂直位置;
     var iLeft = (window.screen.availWidth-10-iWidth)/2; //获得窗口的水平位置;
